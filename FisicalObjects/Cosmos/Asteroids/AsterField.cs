    ...
    static class AsterField
    {
        ...
        public static Point Attack(Point pos, int rang, int demage, int fmass, string hiteffect)
        {
            ...
            for (int j = 0; j < mean.Count; j++)
            {
                ...
                {
                    pos2 = Asteroids[Field[mean[j].X][mean[j].Y].AsterLinks[k]].GetCoords();
                    r2 = Asteroids[Field[mean[j].X][mean[j].Y].AsterLinks[k]].GetRadius() * AttackCoef;
                    ...
                    if (rang + r2 > (int)tlength)
                    {
                        ...
                        {
                            length = tlength;
                            ind = Field[mean[j].X][mean[j].Y].AsterLinks[k];
                        }
                    }
                }
            }
        ...
        public static int TryClash(Point pos, int r, int mass)	// Проверка на столкновение постройки и астероидов
        {
            ...
            for (int j = 0; j < mean.Count; j++)
            {
                for (int k = 0; k < Field[mean[j].X][mean[j].Y].AsterLinks.Count; k++)
                {
                    ...
                    {
                        double vx, vy, v;
                        vx = Asteroids[Field[mean[j].X][mean[j].Y].AsterLinks[k]].GetVX();
                        vy = Asteroids[Field[mean[j].X][mean[j].Y].AsterLinks[k]].GetVY();
                        v = Math.Sqrt(vx * vx + vy * vy);
                        demage += Asteroids[Field[mean[j].X][mean[j].Y].AsterLinks[k]].GetMass() * v * ClashCoef + 2;
                        SingleClash(Field[mean[j].X][mean[j].Y].AsterLinks[k], mass, pos, pos2, r, r2);
                    }
                }
            }
            return (int)(demage);
        ...
        public static void Explode(Point pos, int explforse)
        {
            ...
            for (int j = 0; j < mean.Count; j++)
            {
                ...
                {
                    pos2 = Asteroids[Field[mean[j].X][mean[j].Y].AsterLinks[k]].GetCoords();
                    r2 = Asteroids[Field[mean[j].X][mean[j].Y].AsterLinks[k]].GetRadius() * ExplCoef1;
                    ...
                    if (explforse + r2 > length)
                    {
                        ...
                        Asteroids[Field[mean[j].X][mean[j].Y].AsterLinks[k]].ExplodeHit(pos, demage, mass);
                    }
                }
            }
        }
