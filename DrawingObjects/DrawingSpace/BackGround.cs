...
namespace TheGameDrawing.DrawingSpace
{
    static class BackGround
    {
        ...
        private static void DrawMiniAliens()
        {
            TransAlien[] aliens = AIUnits.GetTransAliens();
            for (int i = 0; i < aliens.Length; i++)
            {
                SurfaceDescription temp = Textures.miniAliens.Get(aliens[i].Radius.ToString()).GetLevelDescription(0);
                int x = (int)(MiniMap.Size * ((aliens[i].Pozition.X * 1.0) / WorkSpace.MapLen) + MiniMap.DX);
                int y = (int)(MiniMap.Size * ((aliens[i].Pozition.Y * 1.0) / WorkSpace.MapLen) + MiniMap.DY);
                Drawing.OurSprite.Draw2D(Textures.miniAliens.Get(aliens[i].Radius.ToString()), Point.Empty, 0, new Point(x, y), Color.White);
            }
        }
    }
}
