...
namespace Logic.Different.StarsSpace
{
    public static class Stars
    {
        ...
        public static void Initialize(int[] minCount, int[] maxCount, int[] size, int[] count, int koefMin, int koefMax, string[] types, int typesCount, double[] outMapRad)
        {
            Size = size;
            Count = count;
            OutMapRad = outMapRad;
            ...
            for (int j = 0; j < AllStars.Length; j++)
            {
                ...
                {
                    AllStars[j].Add(new Star(rand.Next(Count[j]), OutMapRad[j], 0 - OutMapRad[j], 0 - OutMapRad[j], WorkSpace.MapLen + OutMapRad[j], WorkSpace.MapLen + OutMapRad[j]));
                }
            }
        }
